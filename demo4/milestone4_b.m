...
bitloading_flag = 0.99; % If 1 then on-off bitloading is enabled.
...
if bitloading_flag
    ...
    trainStream = ofdm_mod(repmat(trainblock, Lt, 1), N, Lcp, ON_OFF_mask);
...
time = toc;
time_estimate = time/size(CHANNELS, 2);
rx_qam = rx_qam(1:length(qamStream));
